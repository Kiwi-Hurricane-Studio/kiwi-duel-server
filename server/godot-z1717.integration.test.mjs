...
const archive=path.join(project,'docs/generated/z-skill-lifecycle-20260910/native-followups');
const manifest=JSON.parse(fs.readFileSync(path.join(archive,'manifest.json')));
const query=manifest.queries.find(q=>q.label==='white-field-purple1717-after-auto0'&&q.operation==='status');
...
test('two actual Godot TCP clients execute full native1717 prefix and restore every active phase',{
 timeout:90000,skip:!fs.existsSync(godot)?'Godot runtime unavailable':false,
},async t=>{
 const privateRoot=fs.mkdtempSync(path.join(os.tmpdir(),'kiwi-core-tests-z1717-'));
 const evidence=fs.mkdtempSync(path.join(project,'docs/generated/z-skill-integration-20260911/client/real-tcp-'));
 ...
 const sources=['src/domain/match_z_state.gd','src/domain/match_move_planner.gd','src/services/battle_client.gd','src/ui/match_stage_view.gd','tests/z1717_connection_runner.gd','server/custom-match-engine.mjs','server/human-match-service.mjs'];
 ...
 try{
  ...
  const fixture={users,statuses:users.map((user,index)=>service.poll(sessions[index],user)),actions:nativeRecord.all_moves};
  ...
  child=spawn(godot,['--headless','--path',project,'--quit-after','12000','--log-file',path.join(privateRoot,'engine.log'),
   '--script','res://tests/z1717_connection_runner.gd','--','--expected-user-root='+privateRoot,'--z-fixture='+fixturePath],{cwd:project,env,windowsHide:true,stdio:['ignore','pipe','pipe']});
  let lines='';
  ...
  child.stdout.on('data',chunk=>{stdout+=chunk;lines+=chunk;
   while(lines.includes('\n')){const end=lines.indexOf('\n'),line=lines.slice(0,end).trim();lines=lines.slice(end+1);
    try{
     ...
     }else if(line.startsWith('GODOT_Z1717_CONNECTION_TESTS=')){
      assert.equal(proof,null);proof=JSON.parse(line.slice('GODOT_Z1717_CONNECTION_TESTS='.length));
     }
  ...
  const resolved=path.resolve(privateRoot);assert.equal(path.dirname(resolved),path.resolve(os.tmpdir()));assert.ok(path.basename(resolved).startsWith('kiwi-core-tests-z1717-'));
