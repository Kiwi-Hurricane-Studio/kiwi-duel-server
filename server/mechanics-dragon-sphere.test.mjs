...
import {dragonSphereHistoryCases} from '../tests/dragon-sphere-history-cases.mjs';
...
const cases=dragonSphereHistoryCases();
assert.equal(cases.length,252);
...
for(const c of cases)test(`native Dragon legal history ${c.name}`,t=>{
  ...
  const match=service.createMatch('isolated-native-dragon-sphere-history');match.record=structuredClone(c.record);match.turn=c.initial_status.turn;match.phase='started';match.socket={destroyed:false,write(){},destroy(e){errors.push(e?.message)}};t.after(()=>{match.phase='finished'});
  ...
  for(const [index,step] of c.steps.entries()){
    ...
    if(step.action.value.type==='declare_plate'&&[5377,5378,5379,5380,5412,5416,5426,5445].includes(step.action.value.plate_id)){
    ...
    for(const plateId of [5377,5378,5379,5380,5412,5416])for(const p of step.status.pokemon_conditions)assert.equal(sphereSources(match.record,match.positions,match.plateState,p.pokemon_index,plateId,customMatchContract.fieldEdges).length>0,(p.effect.ids.plates??[]).includes(plateId),'native derived aura membership '+plateId+'/'+p.pokemon_index);
  ...
  if(c.scenario==='same-holder')assert.equal(hooks.validatePlateMove(match,match.turn,{selective_side:match.turn,value:{type:'declare_plate',plate_id:5412,value:{type:'select_pokemon_and_declare_aura',pokemon:c.owner}}}),false,'native existing holder cannot receive duplicate Sphere');
});
