...
const cases=['metal-sphere-admission-research.json','metal-sphere-ice-shard-research.json','metal-sphere-recipient-research.json','metal-sphere-immunity-research.json','metal-sphere-damage-colors-research.json','metal-sphere-lifecycle-research.json','metal-sphere-threshold-research.json','zero-damage-win-research.json','metal-sphere-x-attack-research-v2.json','metal-sphere-poison-threshold-research.json','metal-sphere-duplicate-removal-research.json','metal-sphere-steel-aura-research.json','metal-sphere-repeated-threshold-research.json','zero-damage-reductions-research.json','zero-field-damage-research.json','metal-sphere-adjacent-poison-research.json'].flatMap(f=>JSON.parse(readFileSync(new URL('../docs/generated/mechanics-20260913/'+f,import.meta.url),'utf8')).cases.filter(c=>c.ok));
for(const c of cases)test(`native Metal Sphere legal history ${c.name}`,t=>{
  ...
  const match=service.createMatch('isolated-native-metal-sphere-history');match.record=structuredClone(c.record);match.turn=c.initial_status.turn;match.phase='started';match.socket={destroyed:false,write(){},destroy(e){errors.push(e?.message)}};t.after(()=>{match.phase='finished'});
  service.preflightTurnRecord(match);
  for(const [index,step] of c.steps.entries()){
    const start=match.record.all_moves.length;
    if(step.action.value.type==='declare_plate'&&step.action.value.plate_id===5377){
      const targets=step.legal_before.filter(m=>m.value.type==='declare_plate'&&m.value.plate_id===5377).map(m=>m.value.value.pokemon);
      for(let p=0;p<12;p++)assert.equal(hooks.validatePlateMove(match,step.action.selective_side,{selective_side:step.action.selective_side,value:{type:'declare_plate',plate_id:5377,value:{type:'select_pokemon_and_declare_aura',pokemon:p}}}),targets.includes(p),'native Sphere target set');
    ...
    for(const p of step.status.pokemon_conditions)assert.equal(match.plateState.attachments.some(a=>a.pokemon===p.pokemon_index&&a.effect.charge_effect),Boolean(p.effect.charge_effect),'native source charge attachment '+p.pokemon_index);
    for(const p of step.status.pokemon_conditions)assert.equal(metalSphereSources(match.record,match.positions,match.plateState,p.pokemon_index).length>0,(p.effect.ids.plates??[]).includes(5377),'native derived aura membership '+p.pokemon_index);
