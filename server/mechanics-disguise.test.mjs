...
import {disguiseHistoryCases} from '../tests/disguise-history-cases.mjs';
...
const cases=disguiseHistoryCases();
...
for(const c of cases)test(`native Disguise legal history ${c.name}`,t=>{
  ...
  const match=service.createMatch('isolated-native-disguise-history');match.record=structuredClone(c.record);match.turn=c.initial_status.turn;match.phase='started';match.socket={destroyed:false,write(){},destroy(e){errors.push(e?.message)}};t.after(()=>{match.phase='finished'});
  ...
  service.stagePendingKnockouts=(m,state,outcome,targets)=>{
    const action=c.steps[currentStep+1]?.action;
    assert(action&&action.selective_side==='both','original pending disposition exists');
    assert.equal(action.value.type,'knockedout_move','scoped empty Center continuation');
    const selected=targets.findIndex(p=>p.from===action.value.from);assert(selected>=0);
    service.knockoutChoiceSource=n=>{assert(selected<n);return selected;};
    return stageKnockouts(m,state,outcome,targets);
  };
  ...
  for(const [index,step] of c.steps.entries()){
    ...
    }else if(step.action.selective_side==='both' && step.action.value.type==='knockedout_move'){
      const pending=match.pendingKnockouts;assert(pending);
      assert.deepEqual(pending.firstMove,step.action,'original automatic choice');
      const before=snapshot(match);
      assert.equal(service.performPendingKnockouts(match,{...pending},step.action),false,'stale disposition rejected');
      assert.deepEqual(snapshot(match),before,'stale disposition retains complete state');
      assert.equal(service.performPendingKnockouts(match,pending,step.action),true);
      const after=snapshot(match);
      assert.equal(service.performPendingKnockouts(match,pending,step.action),false,'duplicate disposition rejected');
      assert.deepEqual(snapshot(match),after,'duplicate disposition retains complete state');
