...
import {phantomSphereHistoryCases} from '../tests/phantom-sphere-history-cases.mjs';
...
const cases=phantomSphereHistoryCases();
...
for(const c of cases)test(`native Phantom/sleep legal history ${c.name}`,t=>{
  ...
  const match=service.createMatch('isolated-native-phantom-sphere-history');match.record=structuredClone(c.record);match.turn=c.initial_status.turn;match.phase='started';match.socket={destroyed:false,write(){},destroy(e){errors.push(e?.message)}};t.after(()=>{match.phase='finished'});
  ...
  for(const [index,step] of c.steps.entries()){
    ...
    if(step.action.value.type==='declare_plate'&&[5377,5378,5379,5445].includes(step.action.value.plate_id)){
    ...
    for(const plateId of [5377,5378,5379])for(const p of step.status.pokemon_conditions)assert.equal(sphereSources(match.record,match.positions,match.plateState,p.pokemon_index,plateId,customMatchContract.fieldEdges).length>0,(p.effect.ids.plates??[]).includes(plateId),'native derived aura membership '+plateId+'/'+p.pokemon_index);
